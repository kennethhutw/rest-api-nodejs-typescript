...
export class MockCalculatorDao implements CalculatorDao{
    ...
    private toPersistence(calculator:Calculator): MockCalculatorDbData{
        return {
            ...
            ts: calculator.metadata?.timestamp?? MockCalculatorDao.getCurrentTimestamp(),
        }
    }

    private static dbOperatorMapping = {
        '+' : CalculatorCommandOperator.ADD,
        '-' : CalculatorCommandOperator.SUBTRACT,
        '*' : CalculatorCommandOperator.MULTIPLY,
        '/' : CalculatorCommandOperator.DIVIDE,
    }


    private toDomain(calculatorPersistence: MockCalculatorDbData): Calculator{
